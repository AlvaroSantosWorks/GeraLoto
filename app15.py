...
class GeradorLoterias:
    ...
    def conferir_acertos(self, jogo_gerado):
        # Pega o último sorteio do histórico que já carregamos
        if not self.historico_lista:
            return 0
        ultimo_sorteio = set(self.historico_lista[-1])
        # Calcula quantos números do jogo gerado existem no último sorteio
        acertos = len(set(jogo_gerado).intersection(ultimo_sorteio))
        return acertos
...
if not os.path.exists(arquivo_selecionado):
    ...
    if tecnica == "Desdobramento":
        # Slider aparece apenas no Desdobramento
        tamanho_base = st.slider(
            "Quantos números totais na base?", 
            min_value=min_permitido + 1, 
            max_value=max_permitido, 
            value=min_permitido + 1
        )
        
        # O input de bilhete depende da base escolhida
        ...
        # No Jogo Único, não mostramos slider. 
        # O tamanho já é o min_permitido (6 ou 15)
        st.write(f"Modo Jogo Único: Serão gerados bilhetes com {min_permitido} números.")
        tamanho_base = min_permitido
        ...
        if st.button(f"Gerar {qtd_esperada} Jogo(s) 🚀 (Custa {custo_total} Fichas)", use_container_width=True):
            if st.session_state["fichas"] >= custo_total:
                with st.spinner('Analisando e gerando números...'):
                    ...
                    if jogos:
                        # Pega o número EXATO de jogos que foram gerados
                        ...
                        # --- NOVA LÓGICA: CONFERÊNCIA ---
                        resultado_formatado = []
                        for jogo in jogos:
                            # Adiciona o jogo e calcula os acertos
                            acertos = gerador.conferir_acertos(jogo)
                            jogo_dict = {f"Bola {i+1}": num for i, num in enumerate(sorted(jogo))}
                            jogo_dict["Acertos"] = acertos # Nova coluna!
