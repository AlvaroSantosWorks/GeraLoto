...
if not os.path.exists(arquivo_selecionado):
    ...
    # --- LÓGICA DINÂMICA DE INTERVALOS ---
    if tecnica == "Desdobramento":
        # Se for desdobramento, o mínimo sobe um degrau
        min_base = min_permitido + 1
        max_base = max_permitido
    else:
        # Se for jogo simples, usa os valores padrão
        min_base = min_permitido
        max_base = min_permitido # O slider fica fixo no tamanho mínimo
    ...
    # Se for jogo único, o tamanho do bilhete é o mínimo permitido.
    # Se for desdobramento, permitimos que o usuário escolha o tamanho do bilhete
    if tecnica == "Desdobramento":
