...
class GeradorMegaSenaAvancado:
    ...
    def resetar_gerados(self):
        """Apaga o arquivo de jogos gerados para começar do zero."""
        if os.path.exists(self.caminho_gerados):
            ...
            print("\n[SUCESSO] Histórico de jogos gerados apagado. Pode começar uma nova sequência.")
        else:
            print("\n[AVISO] Nenhum arquivo de jogos gerados encontrado para apagar.")
...
def menu_interativo():
    ...
    # Nova opção: Reset
    decisao = input("Deseja apagar os jogos gerados anteriormente e começar do zero? (S/N): ").upper()
    if decisao == 'S':
        gerador.resetar_gerados()

    # 1. Tamanho da Base
    tamanho_base = int(input("\nQuantos números totais você quer no seu fechamento/base? (6 a 15): "))
    if tamanho_base < 6:
        print("Tamanho inválido. Usando o padrão de 6 números.")
        tamanho_base = 6

    tecnica = 'simples'
    tamanho_bilhete = tamanho_base 
