...
class GeradorMegaSenaAvancado:
    ...
    def gerar_jogos(self, tamanho_base=6, tamanho_bilhete=6, tecnica='simples', equilibrar=False, quantidade_pedida=1):
        ...
        if tecnica == 'desdobramento' and tamanho_base > 6:
            # Tenta gerar bases até encontrar uma onde NENHUM dos bilhetes gerados seja repetido
            tentativas = 0
            while len(jogos_validados) < quantidade_pedida and tentativas < 1000:
                ...
                # 2. Gera todos os bilhetes possíveis dessa base
                possiveis_bilhetes = list(itertools.combinations(base, tamanho_bilhete))
                
                # 3. Validação: Checa se ALGUM dos possíveis bilhetes (ou suas sub-combinações de 6) já existe
                bloco_invalido = False
                for bilhete in possiveis_bilhetes:
                    # Se o bilhete for de 6, checa ele mesmo. Se for maior, checa todas as variações de 6
                    sub_combinacoes = list(itertools.combinations(bilhete, 6))
                    for combo in sub_combinacoes:
                        if combo in self.jogos_proibidos:
                            bloco_invalido = True
                            break
                    if bloco_invalido: break
                
                # 4. Se o bloco todo for inédito, aceita
                if not bloco_invalido:
                    jogos_validados.extend(possiveis_bilhetes)
                    # Adiciona tudo ao proibido para não repetir em futuras chamadas
                    for bilhete in possiveis_bilhetes:
                        sub_combinacoes = list(itertools.combinations(bilhete, 6))
                        for combo in sub_combinacoes:
            ...
            while len(jogos_validados) < quantidade_pedida:
                ...
                # Checa se o jogo (ou qualquer sub-combinação de 6) já foi jogado
                sub_combinacoes = list(itertools.combinations(jogo, 6))
                if not any(combo in self.jogos_proibidos for combo in sub_combinacoes):
