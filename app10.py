...
if not os.path.exists(arquivo_selecionado):
    ...
    if tecnica == "Desdobramento":
        # Se o usuário escolher desdobramento, o bilhete deve ser maior que o mínimo permitido 
        # para fazer sentido (ex: na Lotofácil, escolher 16 e desdobrar em 15).
        # Ajustamos o limite para ser no máximo tamanho_base - 1
        max_bilhete = tamanho_base - 1
        
        # Garantimos que o valor inicial não seja maior que o novo máximo
        valor_inicial = min(min_permitido, max_bilhete)
        
        tamanho_bilhete = st.number_input(
            "Tamanho de cada bilhete?", 
            min_value=min_permitido, 
            max_value=max_bilhete, 
            value=valor_inicial
        )
